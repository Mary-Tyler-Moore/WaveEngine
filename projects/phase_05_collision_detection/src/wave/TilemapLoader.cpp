...
constexpr uint32_t kGidFlipMask = 0xE0000000u;
constexpr uint32_t kGidIndexMask = 0x1FFFFFFFu;
...
uint32_t StripGidFlags(uint32_t gid, bool* hadFlip) {
    if ((gid & kGidFlipMask) != 0) {
        if (hadFlip) {
            *hadFlip = true;
        }
    }
...
std::vector<uint8_t> ExtractShoreTileMask(const nlohmann::json& tileset, int tileCount) {
    std::vector<uint8_t> mask;
    if (tileCount <= 0) {
        return mask;
    }
    mask.assign(static_cast<size_t>(tileCount), 0);
    ...
    for (const auto& tile : tileset["tiles"]) {
        ...
        std::string cls;
        if (tile.contains("class") && tile["class"].is_string()) {
            cls = tile["class"].get<std::string>();
        } else if (tile.contains("type") && tile["type"].is_string()) {
            cls = tile["type"].get<std::string>();
        }
        if (cls == "shore") {
...
std::optional<ChunkMap> LoadChunkMapFromJson(const std::filesystem::path& path,
    ...
    std::vector<uint8_t> shoreTileMask;
    for (const auto& tileset : doc["tilesets"]) {
        ...
        shoreTileMask = ExtractShoreTileMask(tileset, *tileCount);
        foundAtlas = true;
        break;
    }
    if (!foundAtlas) {
    ...
    if (!shoreTileMask.empty()) {
        const size_t tileCount =
            static_cast<size_t>(*mapWidth) * static_cast<size_t>(*mapHeight);
        shoreMask.assign(tileCount, 0);
        bool sawFlip = false;
        const uint32_t atlasFirst = static_cast<uint32_t>(atlas.firstGid);
        const uint32_t atlasLast =
            atlasFirst + static_cast<uint32_t>(atlas.tileCount - 1);
        for (const ChunkLayer& layer : layers) {
            if (layer.gids.size() < tileCount) {
                continue;
            }
            for (size_t i = 0; i < tileCount; ++i) {
                uint32_t gid = layer.gids[i];
                if (gid == 0) {
                    continue;
                }
                bool hadFlip = false;
                gid = StripGidFlags(gid, &hadFlip);
                if (hadFlip) {
                    sawFlip = true;
                }
                ...
                const uint32_t local = gid - atlasFirst;
                if (local < shoreTileMask.size() && shoreTileMask[local] != 0) {
                    shoreMask[i] = 1;
                }
            }
        }
        if (sawFlip) {
            ...
            if (!loggedOnce) {
                std::cerr << "Tilemap contains flipped tiles; shoreline collision ignores flips."
                          << "\n";
                loggedOnce = true;
            }
        }
    ...
    map.shoreMask = std::move(shoreMask);
    return map;
}
