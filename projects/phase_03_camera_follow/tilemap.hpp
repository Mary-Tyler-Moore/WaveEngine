...
int RenderTilemap(SDL_Renderer* renderer, SDL_Texture* atlas, const Tilemap& map,
    ...
    SDL_Rect dst{0, 0, map.tileWidth, map.tileHeight};
    int tilesDrawn = 0;
    ...
    for (const TileLayer& layer : map.layers) {
        ...
        for (size_t index = 0; index < tileCount; ++index) {
            ...
            const float worldX = static_cast<float>(tileX * map.tileWidth);
            const float worldY = static_cast<float>(tileY * map.tileHeight);
